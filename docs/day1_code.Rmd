...
# Filter neophyte species only
neo <- species[species$STATUS == "neo", ]
nrow(neo)
```
...
# How many UNIQUE neophyte species?
n_neo <- length(unique(neo$WFO_TAXON))
n_neo

# Ratio
cat("Ratio of native to neo:", round(n_native / n_neo, 1), ": 1\n")
```
...
There are about `r round(n_native / n_neo, 0)` times more native species than neophyte species in our dataset.
...
Es gibt etwa `r round(n_native / n_neo, 0)` mal mehr heimische als Neophyten-Arten in unserem Datensatz.
