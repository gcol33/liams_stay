...
# 1b: Speichere Text (braucht Anführungszeichen!)
...
# 1d: Du bist dran - erstelle einen Vektor mit 4 Baumhöhen

tree_heights <- c(___, ___, ___, ___)
tree_heights

# 1e: Get the length of a vector
# 1e: Hole die Länge eines Vektors
...
# 1g: Berechne den Mittelwert deiner Baumhöhen
...
`unique()` ist die **Schlüsselfunktion** für Ökologie! Sie sagt uns, wie viele *verschiedene* Arten wir gefunden haben.
...
```{r}
# 3a: Load the tidyverse package
# 3a: Lade das tidyverse Paket

library(tidyverse)

# 3b: Load the species data
# 3b: Lade die Artdaten

species_data <- read_csv("../data/austria_species.csv")

# 3c: Look at the first few rows
# 3c: Schau dir die ersten Zeilen an

head(species_data)

# 3d: How many rows (observations)?
# 3d: Wie viele Zeilen (Beobachtungen)?

nrow(species_data)

# 3e: What columns do we have?
# 3e: Welche Spalten haben wir?

colnames(species_data)

# 3f: How many unique plots?
# 3f: Wie viele einzigartige Plots?

length(unique(species_data$PlotObservationID))

# 3g: How many unique species in the whole dataset?
# 3g: Wie viele einzigartige Arten im gesamten Datensatz?
...
```{r}
# 4a: Get a list of all unique plots
# 4a: Hole eine Liste aller einzigartigen Plots

all_plots <- unique(species_data$PlotObservationID)
head(all_plots)

# 4b: Take a small sample (first 50 plots for speed)
# 4b: Nimm eine kleine Stichprobe (erste 50 Plots für Geschwindigkeit)

sample_plots <- all_plots[1:50]

# 4c: Initialize our accumulation tracking
# 4c: Initialisiere unsere Akkumulations-Verfolgung
...
# 4d: Loop through each plot and count species!
# 4d: Schleife durch jeden Plot und zähle Arten!
...
# 4e: Look at the result!
# 4e: Schau dir das Ergebnis an!
...
```{r}
# 5a: Create a data frame for plotting
# 5a: Erstelle einen Data Frame zum Plotten
...
# 5c: Make it prettier with a filled area
# 5c: Mache es schöner mit gefüllter Fläche
...
- `c()` creates vectors
- `unique()` counts different species
...
- `c()` erstellt Vektoren
- `unique()` zählt verschiedene Arten
