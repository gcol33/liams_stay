...
# Check which points are inside Austria using sf
samples_sf <- st_as_sf(samples, coords = c("x", "y"), crs = 4326)
samples$inside <- st_intersects(samples_sf, austria_sf, sparse = FALSE)[, 1]
samples$location <- ifelse(samples$inside, "Inside Austria", "Outside Austria")

ggplot() +
  geom_sf(data = austria_sf, fill = "#C8E6C9", color = "#2E7D32", linewidth = 1.5, alpha = 0.7) +
  ...
  coord_sf(xlim = c(9.3, 17.2), ylim = c(46.2, 49.5))
```
