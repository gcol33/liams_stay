...
# ============================================================
# WHY THIS DESIGN:
# - Takes individual coordinates, not data frames
#   -> More flexible: works with any x,y data
# - Uses Euclidean distance (Pythagorean theorem)
#   -> Simple, fast, good enough for nearby points
# - VECTORIZED: x2,y2 can be vectors!
#   -> Calculate distance to ALL points at once
#
# WARUM DIESES DESIGN:
# - Nimmt einzelne Koordinaten, keine Data Frames
#   -> Flexibler: funktioniert mit beliebigen x,y Daten
# - Nutzt Euklidische Distanz (Satz des Pythagoras)
#   -> Einfach, schnell, gut genug für nahe Punkte
# - VEKTORISIERT: x2,y2 können Vektoren sein!
#   -> Berechne Distanz zu ALLEN Punkten auf einmal
# ============================================================
...
# ============================================================
# FUNCTION 2: nn_walk (Nearest-Neighbour Walk)
# ============================================================
# WHY THIS DESIGN:
# - start_idx = NULL means "pick random start"
#   -> Default is random, but user can control it
# - Returns PlotObservationID, not row indices
#   -> More useful: can directly filter species data
# - Uses "visited" boolean vector + Inf trick
#   -> Elegant way to exclude already-visited plots
# - WHY A LOOP? This is a "greedy algorithm" - each step
#   depends on previous steps. Can't vectorize!
#
# WARUM DIESES DESIGN:
# - start_idx = NULL bedeutet "wähle zufälligen Start"
#   -> Standard ist zufällig, aber Nutzer kann kontrollieren
# - Gibt PlotObservationID zurück, nicht Zeilenindizes
#   -> Nützlicher: kann direkt Artdaten filtern
# - Nutzt "visited" Boolean-Vektor + Inf-Trick
#   -> Elegante Art, bereits besuchte Plots auszuschließen
# - WARUM EINE SCHLEIFE? Dies ist ein "Greedy-Algorithmus" -
#   jeder Schritt hängt von vorherigen ab. Nicht vektorisierbar!
# ============================================================
nn_walk <- function(header_data, start_idx = NULL) {
  ...
  for (i in 1:n) {
    ...
    if (i < n) {
      # VECTORIZED: calculate distance to ALL plots at once
      # VEKTORISIERT: berechne Distanz zu ALLEN Plots auf einmal
...
# ============================================================
# WHY THIS DESIGN:
# - status_filter is OPTIONAL (NULL = use all species)
#   -> Same function works for native, alien, or all
# - Uses setdiff() to find NEW species
#   -> Set operations are fast and clear
# - Returns vector same length as plot_order
#   -> Easy to plot: x = 1:n, y = accumulation
# - WHY A LOOP? Accumulation is inherently sequential:
#   "how many species after plot 1, then 2, then 3..."
#
# WARUM DIESES DESIGN:
# - status_filter ist OPTIONAL (NULL = alle Arten)
#   -> Gleiche Funktion für heimisch, alien, oder alle
# - Nutzt setdiff() um NEUE Arten zu finden
#   -> Mengenoperationen sind schnell und klar
# - Gibt Vektor gleicher Länge wie plot_order zurück
#   -> Einfach zu plotten: x = 1:n, y = Akkumulation
# - WARUM EINE SCHLEIFE? Akkumulation ist sequentiell:
#   "wie viele Arten nach Plot 1, dann 2, dann 3..."
# ============================================================
build_accumulation <- function(species_data, plot_order, status_filter = NULL) {
  ...
  for (i in seq_along(plot_order)) {
    # Get unique species in this plot
    # Hole einzigartige Arten in diesem Plot
    plot_spp <- species_data %>%
      filter(PlotObservationID == plot_order[i]) %>%
      pull(WFO_TAXON) %>%
      unique()

    # setdiff: "what's in plot_spp but NOT in found?"
    # setdiff: "was ist in plot_spp aber NICHT in found?"
...
# ============================================================
# WHY THIS DESIGN:
# - threshold has DEFAULT (0.8 = 80%)
#   -> Common choice, but user can change
# - Returns the FIRST plot where we reach threshold
#   -> which()[1] finds first TRUE in logical vector
# - Works on the curve output from build_accumulation
#   -> Functions chain together nicely
#
# WARUM DIESES DESIGN:
# - threshold hat STANDARD (0.8 = 80%)
#   -> Häufige Wahl, aber Nutzer kann ändern
# - Gibt den ERSTEN Plot zurück wo wir Schwelle erreichen
#   -> which()[1] findet erstes TRUE im logischen Vektor
# - Funktioniert auf der Kurve von build_accumulation
#   -> Funktionen verketten sich schön
# ============================================================
