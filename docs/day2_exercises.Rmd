...
nn_walk <- function(header_data, start_idx = NULL) {
  n <- nrow(header_data)                                    # how many plots total / wie viele Plots insgesamt
  if (is.null(start_idx)) start_idx <- sample(1:n, 1)       # if no start given, pick random / wenn kein Start, wähle zufällig

  visited <- rep(FALSE, n)      # track which plots we've visited / verfolge welche Plots besucht
  visit_order <- numeric(n)     # store the order we visit them / speichere Reihenfolge
  current <- start_idx          # start at this plot / starte bei diesem Plot

  for (i in 1:n) {                                          # loop through all plots / Schleife durch alle Plots
    visited[current] <- TRUE                                # mark current as visited / markiere aktuellen als besucht
    visit_order[i] <- header_data$PlotObservationID[current] # save the plot ID / speichere Plot-ID

    if (i < n) {                                            # if not done yet / falls noch nicht fertig
      distances <- calc_distance(                           # calculate distance from current to ALL others
        header_data$Longitude[current],                     # current plot's longitude / Längengrad aktueller Plot
        header_data$Latitude[current],                      # current plot's latitude / Breitengrad aktueller Plot
        header_data$Longitude,                              # ALL longitudes (vector!) / ALLE Längengrade (Vektor!)
        header_data$Latitude                                # ALL latitudes (vector!) / ALLE Breitengrade (Vektor!)
      )
      distances[visited] <- Inf                             # Inf trick: visited can't be minimum / Inf-Trick: besuchte nie Minimum
      current <- which.min(distances)                       # go to closest unvisited / gehe zum nächsten unbesuchten
    }
  }
  return(visit_order)                                       # return the order of plot IDs / gib Reihenfolge der Plot-IDs zurück
}
...
build_accumulation <- function(species_data, plot_order, status_filter = NULL) {
  if (!is.null(status_filter)) {                            # if filter provided (e.g., "nat") / falls Filter angegeben
    species_data <- species_data %>% filter(STATUS == status_filter)  # keep only matching species / behalte nur passende
  }

  found <- c()                                # empty vector to collect all species found / leerer Vektor für gefundene Arten
  accum <- numeric(length(plot_order))        # pre-allocate result vector / Ergebnisvektor vorbelegen

  for (i in seq_along(plot_order)) {          # for each plot in our walking order / für jeden Plot in unserer Laufreihenfolge
    plot_spp <- species_data %>%              # get species in this plot:
      filter(PlotObservationID == plot_order[i]) %>%  # filter to current plot / filtere auf aktuellen Plot
      pull(WFO_TAXON) %>%                     # extract species names / extrahiere Artnamen
      unique()                                # remove duplicates within plot / entferne Duplikate im Plot

    new_spp <- setdiff(plot_spp, found)       # setdiff: what's NEW? (in plot but not yet found) / was ist NEU?
    found <- c(found, new_spp)                # add new species to our collection / füge neue Arten zur Sammlung
    accum[i] <- length(found)                 # count total species so far / zähle Gesamtarten bisher
  }
  return(accum)                               # return the accumulation curve / gib Akkumulationskurve zurück
}
...
find_saturation <- function(curve, threshold = 0.8) {
  target <- max(curve) * threshold            # calculate target: 80% of final count / berechne Ziel: 80% der Endzahl
  which(curve >= target)[1]                   # which()[1] = first position where TRUE / erste Position wo TRUE
}
