...
# Create grid
grid_x <- seq(9.5, 17, by = 0.5)
grid_y <- seq(46.3, 49, by = 0.3)
grid <- expand.grid(x = grid_x, y = grid_y)
grid$inside <- mapply(point_in_polygon, grid$x, grid$y,
...
grid$alien_richness <- ifelse(grid$inside,
  # Higher near Vienna (16.4, 48.2) and Linz (14.3, 48.3)
  pmax(0,
    25 * exp(-((grid$x - 16.4)^2 + (grid$y - 48.2)^2) / 2) +  # Vienna
    15 * exp(-((grid$x - 14.3)^2 + (grid$y - 48.3)^2) / 3) +  # Linz
    8 * exp(-((grid$x - 13)^2 + (grid$y - 47.8)^2) / 4) +     # Salzburg
    rnorm(nrow(grid), 3, 2)
  ),
  NA
)
...
grid$native_richness <- ifelse(grid$inside,
  pmax(0,
    40 * exp(-((grid$x - 11)^2 + (grid$y - 47)^2) / 3) +      # Tyrol Alps
    35 * exp(-((grid$x - 13.5)^2 + (grid$y - 47.2)^2) / 4) +  # Hohe Tauern
    30 * exp(-((grid$x - 15)^2 + (grid$y - 46.8)^2) / 3) +    # Carinthia
...
p1 <- ggplot() +
  # Austria outline
  geom_polygon(data = austria, aes(x, y), fill = "white", color = "#666666", linewidth = 1) +
  # Grid cells colored by alien richness
  geom_tile(data = grid_inside, aes(x = x, y = y, fill = alien_richness),
            width = 0.5, height = 0.3, alpha = 0.8) +
  # Austria outline on top
  geom_polygon(data = austria, aes(x, y), fill = NA, color = "#333333", linewidth = 1.2) +
  # Grid lines
  geom_vline(xintercept = grid_x, color = "gray50", linewidth = 0.2, alpha = 0.5) +
  geom_hline(yintercept = grid_y, color = "gray50", linewidth = 0.2, alpha = 0.5) +
  # City labels
  annotate("point", x = 16.4, y = 48.2, size = 3, color = "black") +
  annotate("text", x = 16.4, y = 48.5, label = "Vienna", size = 3, fontface = "bold") +
  annotate("point", x = 14.3, y = 48.3, size = 2, color = "black") +
  annotate("text", x = 14.3, y = 48.55, label = "Linz", size = 2.5) +
  ...
  coord_fixed(ratio = 1.4, xlim = c(9.3, 17.2), ylim = c(46.2, 49.2)) +
...
p2 <- ggplot() +
  ...
  annotate("text", x = 11, y = 46.7, label = "Tyrol", size = 3, fontface = "italic") +
  annotate("text", x = 13.5, y = 46.9, label = "Hohe Tauern", size = 2.5, fontface = "italic") +
