...
```{r pooled-curves, fig.height=5, fig.width=8}
...
plot_normalized(Native = sac_native, Alien = sac_alien,
                title = "Native vs Alien Accumulation (all decades, normalized)") +
  scale_color_manual(values = c("Native" = "#2E7D32", "Alien" = "#D32F2F")) +
  scale_fill_manual(values = c("Native" = "#2E7D32", "Alien" = "#D32F2F"))
```

```{r statistical-test, fig.height=5, fig.width=8}
compare <- readRDS(file.path(comp_dir, "compare.rds"))
print(compare)

# Custom permutation test plot: normalized curves with CI
s_native <- summary(sac_native)
s_alien  <- summary(sac_alien)

df_perm <- rbind(
 data.frame(
    prop_sites = s_native$sites / max(s_native$sites),
    mean = s_native$mean / max(s_native$mean),
    lower = s_native$lower / max(s_native$mean),
    upper = s_native$upper / max(s_native$mean),
    group = "Native"
  ),
  data.frame(
    prop_sites = s_alien$sites / max(s_alien$sites),
    mean = s_alien$mean / max(s_alien$mean),
    lower = s_alien$lower / max(s_alien$mean),
    upper = s_alien$upper / max(s_alien$mean),
    group = "Alien"
  )
)

ggplot(df_perm, aes(x = prop_sites, y = mean, colour = group, fill = group)) +
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "Permutation Test: Native vs Alien Accumulation",
    subtitle = sprintf("p = %.3f (permutation test, %d iterations)",
                       compare$p_value, compare$n_perm),
    x = "Proportion of sites visited",
...
Does the spatial accumulation pattern change over time? Here we run the same analysis separately for each decade, splitting native and alien species into separate panels. Each curve is normalized by its total species count.

</div>

<div class="de">

Verändert sich das räumliche Akkumulationsmuster über die Zeit? Hier führen wir dieselbe Analyse getrennt für jedes Jahrzehnt durch, wobei heimische und gebietsfremde Arten in separate Panels aufgeteilt werden. Jede Kurve ist nach ihrer Gesamtartenzahl normalisiert.

</div>

```{r decade-curves, fig.height=6, fig.width=12}
decade_na <- readRDS(file.path(comp_dir, "decade_na_results.rds"))

# Build normalized curves for each decade, split by native/alien
df_decades <- do.call(rbind, lapply(names(decade_na), function(d) {
  res <- decade_na[[d]]

  df_list <- lapply(c("native", "alien"), function(g) {
    sac <- res[[g]]
    s <- summary(sac)
    total <- max(s$mean)
    if (total == 0) return(NULL)
    data.frame(
      prop_sites   = s$sites / max(s$sites),
      prop_species = s$mean / total,
      decade       = d,
      n_sites      = res$n_sites,
      group        = ifelse(g == "native", "Native", "Alien")
    )
  })
  do.call(rbind, df_list[!vapply(df_list, is.null, logical(1))])
}))

# Order decades chronologically
decade_order <- c("pre-1970", "1970s", "1980s", "1990s", "2000s", "2010s")
df_decades$decade <- factor(df_decades$decade, levels = decade_order)

# Colour gradients: green for native, red for alien
native_colours <- c(
  "pre-1970" = "#C8E6C9", "1970s" = "#A5D6A7", "1980s" = "#66BB6A",
  "1990s" = "#2E7D32", "2000s" = "#1B5E20", "2010s" = "#0D3B0F"
)
alien_colours <- c(
  "pre-1970" = "#FFCDD2", "1970s" = "#EF9A9A", "1980s" = "#E57373",
  "1990s" = "#D32F2F", "2000s" = "#B71C1C", "2010s" = "#7F0000"
)

# Create interaction for colour mapping
df_decades$decade_group <- interaction(df_decades$decade, df_decades$group, sep = "_")

# Build colour vector
all_colours <- c(
  sapply(decade_order, function(d) setNames(native_colours[d], paste0(d, "_Native"))),
  sapply(decade_order, function(d) setNames(alien_colours[d], paste0(d, "_Alien")))
)

# Labels with sample size
site_counts <- sapply(decade_order, function(d) decade_na[[d]]$n_sites)
decade_labels <- paste0(decade_order, " (n=", site_counts, ")")
names(decade_labels) <- decade_order

ggplot(df_decades, aes(x = prop_sites, y = prop_species,
                       colour = decade_group, linewidth = decade)) +
  geom_line(alpha = 0.85) +
  facet_wrap(~ group) +
  scale_colour_manual(values = all_colours, guide = "none") +
  scale_linewidth_manual(values = c("pre-1970" = 0.5, "1970s" = 0.6, "1980s" = 0.7,
                                    "1990s" = 1.0, "2000s" = 0.8, "2010s" = 0.7),
