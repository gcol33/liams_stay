...
```{r pooled-vs-grouped, fig.height=5, fig.width=12}
# Load both datasets
...
# Build pooled data frame
build_pooled_df <- function(sac, group_name) {
  s <- summary(sac)
  total <- max(s$mean)

  data.frame(
    sites = s$sites, mean = s$mean / total,
    lower = s$lower / total, upper = s$upper / total,
    group = group_name
  )
}
df_pooled <- rbind(

  build_pooled_df(sac_native, "Native"),
  build_pooled_df(sac_alien, "Alien")
)
df_pooled$method <- "Separate runs"

# Build grouped data frame
summaries <- summary(sac_grouped)
df_list <- lapply(names(summaries), function(g) {
  s <- summaries[[g]]
  if (length(s$sites) == 0) return(NULL)
  total <- max(s$mean)
  ...
  data.frame(
    sites = s$sites, mean = s$mean / total,
    lower = s$lower / total, upper = s$upper / total,
    group = ifelse(g == "native", "Native", "Alien")
  )
})
df_list <- df_list[!vapply(df_list, is.null, logical(1))]
df_grouped <- do.call(rbind, df_list)
df_grouped$method <- "Same spatial walk"

# Combine
df_both <- rbind(df_pooled, df_grouped)
df_both$method <- factor(df_both$method, levels = c("Separate runs", "Same spatial walk"))

ggplot(df_both, aes(x = sites, y = mean, colour = group, fill = group)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2, colour = NA) +
  geom_line(linewidth = 1) +
  facet_wrap(~ method) +
  scale_y_continuous(labels = scales::percent) +
  scale_colour_manual(values = c("Native" = "#2E7D32", "Alien" = "#D32F2F")) +
  scale_fill_manual(values = c("Native" = "#2E7D32", "Alien" = "#D32F2F")) +
  labs(
    title = "Native vs Alien Accumulation: Two Approaches (1990s)",
    subtitle = "Left: independent spatial walks | Right: both groups tracked on the same walk",
...
**Why two methods?** The left panel runs separate spatial accumulations for native and alien species, each finding its own optimal path through the landscape. The right panel tracks both groups along the *same* spatial walk, ensuring a fair comparison under identical sampling order. If the two panels look similar, the result is robust. If they differ, the "same spatial walk" version is more rigorous because it controls for spatial autocorrelation. We use the 1990s here as the reference decade due to its large sample size.

</div>

<div class="de">

**Warum zwei Methoden?** Das linke Panel führt separate räumliche Akkumulationen für heimische und gebietsfremde Arten durch, wobei jede ihren eigenen optimalen Pfad durch die Landschaft findet. Das rechte Panel verfolgt beide Gruppen entlang des *gleichen* räumlichen Pfads und gewährleistet so einen fairen Vergleich unter identischer Stichprobenreihenfolge. Wenn beide Panels ähnlich aussehen, ist das Ergebnis robust. Falls sie sich unterscheiden, ist die Version mit "gleichem räumlichen Pfad" rigoroser, da sie die räumliche Autokorrelation kontrolliert. Wir verwenden hier die 1990er Jahre als Referenzdekade aufgrund ihrer großen Stichprobengröße.
