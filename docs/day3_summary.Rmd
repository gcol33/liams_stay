...
```{r beta-diversity, fig.height=6, fig.width=10}
beta <- readRDS(file.path(comp_dir, "beta.rds"))

# Build data frame from the three beta components
n <- ncol(beta$beta_total)
df_beta <- data.frame(
  sites     = rep(seq_len(n), 3),
  value     = c(colMeans(beta$beta_turnover),
                colMeans(beta$beta_nestedness),
                colMeans(beta$beta_total)),
  component = factor(
    rep(c("Turnover (bsim)", "Nestedness (bnes)", "Total (bsor)"), each = n),
    levels = c("Total (bsor)", "Turnover (bsim)", "Nestedness (bnes)")
  )
)

# Panel plot: each component on its own y-axis scale
ggplot(df_beta, aes(x = sites, y = value, colour = component)) +
  geom_line(linewidth = 0.9) +
  facet_wrap(~ component, scales = "free_y", ncol = 1) +
  scale_colour_manual(values = c(
    "Total (bsor)"       = "#1565C0",
    "Turnover (bsim)"    = "#E65100",
    "Nestedness (bnes)"  = "#6A1B9A"
  )) +
  labs(
    title = "Beta Diversity Decomposition: Turnover vs Nestedness (1990s)",
    subtitle = "Baselga (2010): bsor = bsim + bnes. Each panel uses its own y-axis.",
...
```{r hill-numbers, fig.height=5}
hill <- readRDS(file.path(comp_dir, "hill.rds"))
plot(hill) +
  ggtitle("Hill Number Accumulation (1990s, q = 0, 1, 2)") +
  theme_minimal(base_size = 13)
```
...
ggplot(df_hill, aes(x = sites, y = value, colour = group)) +
  ...
  labs(
    ...
    subtitle = "Each panel shows a different diversity order (free y-axis) / Jedes Panel zeigt eine andere Diversitätsordnung",
    x = "Number of sites in spatial walk / Anzahl Standorte",
...
ggplot(df_phylo_all, aes(x = sites, y = value, colour = metric)) +
  ...
  labs(
    ...
    subtitle = "Each panel uses its own y-axis / Jedes Panel hat eine eigene y-Achse",
    x = "Number of sites in spatial walk / Anzahl Standorte",
...
ggplot(df_phylo, aes(x = sites, y = mpd, colour = group)) +
  ...
  labs(
    ...
    subtitle = "Each panel uses its own y-axis scale / Jedes Panel hat eine eigene y-Achsen-Skalierung",
...
**1. Geografie.** Die Hotspot-Karten zeigen, dass heimischer und Alien-Artenreichtum verschiedenen räumlichen Regeln folgen. Einheimische erreichen ihren Höhepunkt in den topografisch komplexen Voralpen; Aliens konzentrieren sich um menschliche Infrastruktur im Tiefland.
