...
for (d in decades) {
  print(plot(decade_na_results[[d]]) +
          ggtitle(sprintf("%s: Native vs Alien", d)))
}
```
...
```{r plot-pooled}
combined <- c(Native = sac_native, Alien = sac_alien, All = sac_all)
plot(combined)
```
...
for (h in keep_habs) {
  ...
  comb <- c(Native = hab_na_results[[h]]$native,
            Alien  = hab_na_results[[h]]$alien)
  print(plot(comb) + ggtitle(sprintf("Habitat %s: Native vs Alien", h)))
}
```
